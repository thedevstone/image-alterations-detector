import cv2
import matplotlib.pyplot as plt

from face_align.face_aligner import FaceAligner
from feature_extraction.landmarks.landmark_extractor import LandmarkExtractor
from feature_extraction.utils.plotting import get_images_mosaic

if __name__ == '__main__':
    landmark_extractor = LandmarkExtractor("../../shape_predictor_68_face_landmarks.dat")
    face_aligner = FaceAligner()

    # Images
    img1 = cv2.imread('../../../images/img1.jpg', cv2.IMREAD_COLOR)
    img2 = cv2.imread('../../../images/img2_rotated.jpg', cv2.IMREAD_COLOR)
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
    img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)

    shape = landmark_extractor.get_2d_landmarks(img1)
    aligned_image = face_aligner.align(img1, shape)
    images_to_show = [(img1, "Img"),
                      (aligned_image, "Aligned")]
    mosaic = get_images_mosaic("Alignment", images_to_show, rows=1, cols=2)
    plt.show()
